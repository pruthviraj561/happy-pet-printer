...
class UsbPrinterConnection(
    ...
    override val isConnected: Boolean get() = active
    ...
    override fun disconnect() {
        try { if (active) connection.releaseInterface(printerInterface) } catch (_: Exception) {}
        active = false
        try { connection.close() } catch (_: Exception) {}
        onDisconnected()
    }
        ...
        Thread {
            try {
                ...
                while (offset < data.size) {
                    ...
                    val sent = connection.bulkTransfer(outEndpoint, data, offset, length, 5000)
                    if (sent <= 0) throw IOException("USB printer did not accept print data")
                    offset += sent
                }
            } catch (e: Exception) {
